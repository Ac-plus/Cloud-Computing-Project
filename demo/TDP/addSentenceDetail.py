...
def tokenize(text: str) :
    ...
    '''
    # 创建分词器
    tokenizer = RegexpTokenizer(r'\S+@\S+|\S+://\S+|\w+|\S+')
    # 对每个文档进行分词
    documents = []
    documents.append(text)
    tokens = [tokenizer.tokenize(doc.lower()) for doc in documents]
    #print(type(tokens[0]))
    return tokens
    '''
...
def addSentenceDetails(documents: str) -> pd.DataFrame:
    """
    ...
    # 将结果转换为DataFrame格式
    df = pd.DataFrame(tokens, columns=['Token', 'DocumentNumber', 'LineNumber', 'SentNumber', 'Type', 'Language'])
...
'''df = addSentenceDetails('./test.txt')
